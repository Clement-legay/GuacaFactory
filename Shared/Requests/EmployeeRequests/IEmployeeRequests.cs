...
public interface IEmployeeRequests
{
    ...
    Task<ICollection<Employee>?> GetEmployeesBySiteIdAsync(int siteId, string search, int page, int rows);
    Task<ICollection<Employee>?> GetEmployeesByServiceIdAsync(int serviceId, string search, int page, int rows);
